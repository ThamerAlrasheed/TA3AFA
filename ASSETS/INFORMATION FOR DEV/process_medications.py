...
def is_match(med, prod):
    ...
    if len(words) > 0:
        # Match if the first significant word is in the product name
        if words[0] in prod:
            return True
    return False
...
print(f"Matched {len(matched_indices)} rows out of {len(moh_df)}")

# Filter the original dataframe using the matched indices
filtered_df = moh_df.loc[matched_indices].copy()

# Drop the temporary clean columns to keep NOTHING MORE NOTHING LESS
filtered_df = filtered_df.drop(columns=['PRODUCT_NAME_CLEAN', 'MANUFACTURER_CLEAN'])

# Overwrite the original MOH file with the filtered data
filtered_df.to_excel(moh_file, index=False)
print(f"Successfully saved {len(filtered_df)} medications into {moh_file}.")
